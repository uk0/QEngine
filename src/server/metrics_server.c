...
static void *handle_connection(void *arg) {
    ...
    if (route_login == 1) {
        ...
        static const char FORM[] =
            ...
            "body{margin:0;font:14px/1.5 system-ui,-apple-system,Segoe UI,Roboto,sans-serif;color:#0f172a}"
            ".wrap{display:flex;min-height:100vh}"
            ".hero{flex:1;position:relative;overflow:hidden;color:#fff;padding:44px;display:flex;"
            "flex-direction:column;justify-content:space-between;"
            "background:radial-gradient(130% 110% at 18% 12%,#4f46e5 0%,#4338ca 40%,#312e81 70%,#1e1b4b 100%)}"
            ".hero-brand{position:relative;z-index:2;display:flex;align-items:center;gap:12px}"
            ".hero-brand img{width:34px;height:34px;border-radius:9px;display:block}"
            ".hero-brand b{font-size:19px}"
            ".hero-copy{position:relative;z-index:2;max-width:400px}"
            ".hero-copy h2{font-size:27px;line-height:1.22;margin:0 0 12px;font-weight:640}"
            ".hero-copy p{margin:0;color:rgba(226,232,255,.72);font-size:13.5px;max-width:360px}"
            ".graph{position:absolute;inset:0;width:100%;height:100%;z-index:1}"
            ".edge{stroke:rgba(199,210,254,.16);stroke-width:1.1;fill:none}"
            ".flow{stroke:#a5b4fc;stroke-width:1.7;fill:none;stroke-linecap:round;"
            "stroke-dasharray:2 12;animation:flow 2.1s linear infinite}"
            "@keyframes flow{to{stroke-dashoffset:-14}}"
            ".node{fill:#c7d2fe}.node.on{fill:#e0e7ff;animation:blink 3s ease-in-out infinite}"
            "@keyframes blink{0%,100%{opacity:.55}50%{opacity:1}}"
            ".hub-core{fill:#fff}.hub-ring{fill:none;stroke:#a5b4fc;stroke-width:1.4}"
            ".p1{animation:pulse 3s ease-out infinite}.p2{animation:pulse 3s ease-out infinite 1.5s}"
            "@keyframes pulse{0%{r:16;opacity:.6}100%{r:60;opacity:0}}"
            ".side{width:440px;display:flex;align-items:center;justify-content:center;background:#f8fafc;padding:24px}"
            ".card{width:320px}.card h1{margin:0 0 4px;font-size:21px}"
            ".card .sub{margin:0 0 22px;color:#64748b;font-size:12.5px}"
            "label{display:block;font-size:12px;color:#334155;margin:14px 0 5px;font-weight:500}"
            "input{width:100%;padding:10px 12px;border:1px solid #cbd5e1;border-radius:8px;font-size:14px;background:#fff;color:#0f172a}"
            ...
            "color:#fff;font-size:14px;font-weight:600;cursor:pointer}button:hover{background:#4f46e5}"
            ".err{margin-top:12px;padding:9px 12px;border-radius:8px;background:#fee2e2;color:#991b1b;"
            "font-size:12.5px;display:none}.err.on{display:block}"
            ".foot{margin-top:20px;color:#94a3b8;font-size:11px}"
            "@media (max-width:860px){.hero{display:none}.side{flex:1;width:auto}}"
            "@media (prefers-color-scheme:dark){.side{background:#0b1220}.card h1{color:#e2e8f0}"
            ".card .sub{color:#94a3b8}label{color:#cbd5e1}input{background:#111a2e;border-color:#334155;color:#e2e8f0}}"
            "</style></head><body><div class=wrap>"
            "<div class=hero>"
            "<svg class=graph viewBox='0 0 640 760' preserveAspectRatio='xMidYMid slice' aria-hidden=true>"
            "<line class=edge x1=120 y1=130 x2=300 y2=90 /><line class=edge x1=520 y1=140 x2=590 y2=330 />"
            "<line class=edge x1=300 y1=650 x2=500 y2=610 /><line class=edge x1=70 y1=300 x2=140 y2=520 />"
            "<line class=edge x1=120 y1=130 x2=380 y2=390 /><line class=edge x1=70 y1=300 x2=380 y2=390 />"
            "<line class=edge x1=140 y1=520 x2=380 y2=390 /><line class=edge x1=300 y1=90 x2=380 y2=390 />"
            "<line class=edge x1=520 y1=140 x2=380 y2=390 /><line class=edge x1=590 y1=330 x2=380 y2=390 />"
            "<line class=edge x1=300 y1=650 x2=380 y2=390 /><line class=edge x1=500 y1=610 x2=380 y2=390 />"
            "<line class=flow x1=120 y1=130 x2=380 y2=390 />"
            "<line class=flow x1=70 y1=300 x2=380 y2=390 style='animation-delay:.4s' />"
            "<line class=flow x1=140 y1=520 x2=380 y2=390 style='animation-delay:.8s' />"
            "<line class=flow x1=300 y1=90 x2=380 y2=390 style='animation-delay:.2s' />"
            "<line class=flow x1=520 y1=140 x2=380 y2=390 style='animation-delay:1s' />"
            "<line class=flow x1=590 y1=330 x2=380 y2=390 style='animation-delay:.6s' />"
            "<line class=flow x1=300 y1=650 x2=380 y2=390 style='animation-delay:1.2s' />"
            "<line class=flow x1=500 y1=610 x2=380 y2=390 style='animation-delay:.9s' />"
            "<circle class='node on' cx=120 cy=130 r=6 /><circle class=node cx=70 cy=300 r=5 />"
            "<circle class='node on' cx=140 cy=520 r=6 /><circle class=node cx=300 cy=90 r=5 />"
            "<circle class='node on' cx=520 cy=140 r=6 /><circle class=node cx=590 cy=330 r=5 />"
            "<circle class=node cx=300 cy=650 r=5 /><circle class='node on' cx=500 cy=610 r=6 />"
            "<circle class='hub-ring p1' cx=380 cy=390 r=16 /><circle class='hub-ring p2' cx=380 cy=390 r=16 />"
            "<circle cx=380 cy=390 r=20 fill=none stroke='rgba(165,180,252,.5)' stroke-width=1.4 />"
            "<circle class=hub-core cx=380 cy=390 r=11 />"
            "</svg>"
            "<div class=hero-brand><img src=/logo.png alt=QEngine><b>QEngine</b></div>"
            "<div class=hero-copy><h2>Everything connected,<br>ingested in real time.</h2>"
            "<p>A nanosecond columnar time-series engine — millions of devices, metrics and events "
            "stream into one replicated store, queried the instant they land.</p></div>"
            "</div>"
            "<div class=side><div class=card>"
            "<h1>Sign in</h1>"
            "<p class=sub>QEngine console — default <b>root / 123456</b> on fresh installs.</p>"
            ...
            "<div class=err id=e>Invalid credentials &mdash; check user / password.</div>"
            "</form>"
            "<div class=foot>cookie session &middot; secured by RBAC</div>"
            "</div></div>"
            "</div>"
            "<script>"
