...
"en:{"
 ...
 "'thead.id':'id','thead.addr':'addr','thead.state':'state',"
 ...
 "'qex.cat.select':'SELECT & filter','qex.cat.agg':'Aggregates',"
 "'qex.cat.win':'Time-bucket / window','qex.cat.join':'Join / latest-on',"
 "'qex.cat.ddl':'DDL — tables','qex.cat.stable':'STables & devices',"
 "'qex.cat.cluster':'Cluster catalog','qex.cat.rbac':'Users & RBAC',"
 "'qex.cat.tmq':'TMQ (pub/sub)','qex.cat.udf':'UDF','qex.cat.export':'Export',"
 ...
 "'common.tables':'Tables','common.groups':'Groups',"
...
"zh:{"
 ...
 "'thead.id':'ID','thead.addr':'地址','thead.state':'状态',"
 "'thead.disk':'磁盘 剩余/总量','thead.used':'使用率',"
 "'thead.vn':'VN 权重','thead.uptime':'运行时长','thead.hb':'最近心跳',"
 "'qex.title':'QTL 示例','qex.hint':'（点击卡片加载到编辑器）',"
 "'qex.cat.select':'SELECT 与过滤','qex.cat.agg':'聚合函数',"
 "'qex.cat.win':'时间桶 / 窗口','qex.cat.join':'Join / Latest-On',"
 "'qex.cat.ddl':'DDL — 表','qex.cat.stable':'超级表与设备',"
 "'qex.cat.cluster':'集群目录','qex.cat.rbac':'用户与 RBAC',"
 "'qex.cat.tmq':'TMQ 消息（发布/订阅）','qex.cat.udf':'UDF 用户函数',"
 "'qex.cat.export':'导出',"
 ...
 "'common.tables':'数据表','common.groups':'分组',"
...
"   '<tr><td colspan=8>'+esc(t('common.no_rows'))+'</td></tr>';"
"}catch(e){"
" document.getElementById('cbody').innerHTML="
"   '<tr><td colspan=8 class=sub>/cluster '+esc(t('status.unreach'))"
"   +': '+e.message+'</td></tr>';}}"
...
/* Render tree from cached state.lastTree — 3-level hierarchy:
 *   Database → Group (stable) → Device (child table).
 * Legacy tables that aren't bound to any stable go in a flat "Tables"
...
"const vtByDb={};"
"vts.forEach(v=>{const k=v.database||'';(vtByDb[k]=vtByDb[k]||[]).push(v);});"
"const ptByVt={};"
"pts.forEach(p=>{const k=p.vtable||'';(ptByVt[k]=ptByVt[k]||[]).push(p);});"
/* ---- Databases section ---- */
"h+='<div class=grp><span>'+esc(t('tree.databases'))+' ('+(dbs.length+((grpsByDb['']||[]).length?1:0))+')</span></div>';"
...
" const gkey=g.name;"
" const devs=devsByGrp[gkey]||[];"
" let gh=`<div class=\"item gr node\" data-nodeid=grp_${idx}>`"
"   +'<span class=ic>G</span>'"
"   +`<span class=nm>${esc(g.name)}</span>`"
"   +`<span class=sz>${devs.length} ${esc(t('tree.lbl_devs'))}</span>`"
"   +`<span class=x data-dropgrp='${esc(g.name)}' title='DROP GROUP'>✕</span>`"
"   +'</div><div class=kids>';"
" if(!devs.length){gh+='<div class=empty>'+esc(t('tree.no_devs'))+'</div>';}"
" else{devs.forEach(d=>{"
...
" const nChildren=vs.length+gs.length;"
...
"                :`<span class=sz>${vs.length} ${esc(t('tree.lbl_vtables'))}"
"                   · ${gs.length} ${esc(t('tree.lbl_groups'))}</span>`)"
...
" }else{"
"   dh+='<div class=grp><span>'+esc(t('tree.vtables'))+' ('+vs.length+')</span></div>';"
"   if(!vs.length)dh+='<div class=empty>'+esc(t('tree.no_vtables'))+'</div>';"
"   else vs.forEach((v,vi)=>{dh+=renderVtable(v,idx*1000+vi);});"
"   if(gs.length){"
"     dh+='<div class=grp><span>'+esc(t('common.groups'))+' ('+gs.length+')</span></div>';"
"     gs.forEach((g,gi)=>{dh+=renderGroup(g,idx*1000+gi);});"
"   }"
" }"
" dh+='</div>';"
" return dh;"
 ...
 * and is_window_call.  Grouped by use-case.  Category titles come from
 * the i18n dictionary (qex.cat.*); item titles carry {en, zh} inline. */
"const QEX=[{c:'qex.cat.select',i:["
...
"]},{c:'qex.cat.agg',i:["
"[{en:'count & sum',zh:'计数 & 求和'},'SELECT count(*), sum(volume), avg(price) FROM trades'],"
"[{en:'min/max/spread',zh:'最小/最大/极差'},'SELECT min(price), max(price), spread(price) FROM trades'],"
"[{en:'first & last',zh:'首值/末值'},'SELECT first(price), last(price), twa(price) FROM trades'],"
...
"]},{c:'qex.cat.join',i:["
"[{en:'latest-on',zh:'latest-on'},'SELECT * FROM trades LATEST ON ts'],"
"[{en:'latest-on partition',zh:'分区 latest-on'},'SELECT * FROM trades LATEST ON ts PARTITION BY symbol'],"
...
"[{en:'create hourly',zh:'按小时分区'},'CREATE TABLE sensor_h (ts TIMESTAMP, v INT64) TIMESTAMP(ts) WITH (PARTITION=\\'hour\\')'],"
