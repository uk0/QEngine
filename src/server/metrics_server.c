...
"</div>';"
"h+='</div>';"
/* ---- Group + device buckets by database / group ---- */
"const grpsByDb={};"
...
" return gh;"
"}"
"function renderDb(dbName,desc,idx,isOrphan){"
...
"   +`<span class=sz>${gs.length} ${esc(t('tree.lbl_groups'))}</span>`"
"   +(isOrphan?''"
"     :`<span class=x data-dropdb='${esc(dbName)}' title='DROP DATABASE'>✕</span>`)"
"   +'</div><div class=kids>';"
" if(!gs.length){dh+='<div class=empty>'+esc(t('tree.no_grps'))+'</div>';}"
" else gs.forEach((g,gi)=>{dh+=renderGroup(g,idx*1000+gi);});"
" dh+='</div>';"
" return dh;"
"}"
"if(!dbs.length && !((grpsByDb['']||[]).length))"
"h+='<div class=empty>'+esc(t('tree.no_dbs'))+'</div>';"
"else{"
" dbs.forEach((d,i)=>{h+=renderDb(d.name,d.description,i+1,false);});"
" if((grpsByDb['']||[]).length)h+=renderDb('',null,0,true);"
"}"
