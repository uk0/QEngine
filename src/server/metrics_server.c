...
"<summary>QTL examples <span class=hint>(click any to load into editor)</span></summary>"
...
/* ---- QTL examples ---------------------------------------------------
 * Full reference derived from src/query/ast.h (every QAST_STMT_* kind)
 * plus the aggregate / window / TS functions recognised by is_agg_call
 * and is_window_call.  Grouped by use-case.  Click any card → editor
 * gets the SQL; the user hits Ctrl+Enter to run. */
"const QEX=[{c:'SELECT & filter',i:["
"['all rows','SELECT * FROM trades LIMIT 100'],"
"['project cols','SELECT ts, symbol, price FROM trades LIMIT 100'],"
"['where range','SELECT * FROM trades WHERE ts BETWEEN \\'2026-01-01\\' AND \\'2026-01-02\\' LIMIT 100'],"
"['where eq','SELECT ts, price FROM trades WHERE symbol = \\'AAPL\\' LIMIT 100'],"
"['distinct','SELECT DISTINCT symbol FROM trades'],"
"['limit + order','SELECT * FROM trades ORDER BY ts DESC LIMIT 10'],"
"]},{c:'Aggregates',i:["
"['count & sum','SELECT count(*), sum(volume), avg(price) FROM trades'],"
"['min/max/spread','SELECT min(price), max(price), spread(price) FROM trades'],"
"['first & last','SELECT first(price), last(price), twa(price) FROM trades'],"
"['quantiles','SELECT p50(price), p90(price), p99(price), stddev(price) FROM trades'],"
"['percentile(col,q)','SELECT percentile(price, 0.95) FROM trades'],"
"['last_row','SELECT last_row(price) FROM trades'],"
"['group by','SELECT symbol, avg(price), count(*) FROM trades GROUP BY symbol'],"
"]},{c:'Time-bucket / window',i:["
"['sample by 1m','SELECT time_bucket(ts, 1m), avg(price) FROM trades SAMPLE BY 1m LIMIT 60'],"
"['session window','SELECT count(*) FROM trades SESSION(ts, 3s)'],"
"['state window','SELECT count(*) FROM trades STATE_WINDOW(status)'],"
"['event window','SELECT count(*) FROM trades EVENT_WINDOW(temp > 20, temp < 15)'],"
"['diff','SELECT ts, diff(price) FROM trades LIMIT 100'],"
"['derivative','SELECT ts, derivative(price) FROM trades LIMIT 100'],"
"['cumulative sum','SELECT ts, csum(volume) FROM trades LIMIT 100'],"
"['moving avg','SELECT ts, mavg(price, 10) FROM trades LIMIT 100'],"
"]},{c:'Join / latest-on',i:["
"['latest-on','SELECT * FROM trades LATEST ON ts'],"
"['latest-on partition','SELECT * FROM trades LATEST ON ts PARTITION BY symbol'],"
"['asof join','SELECT * FROM trades ASOF JOIN quotes ON sym = sym LIMIT 10'],"
"]},{c:'DDL — tables',i:["
"['create table','CREATE TABLE sensor (ts TIMESTAMP, dev SYMBOL, v FLOAT64) TIMESTAMP(ts)'],"
"['create hourly','CREATE TABLE sensor_h (ts TIMESTAMP, v INT64) TIMESTAMP(ts) WITH (PARTITION=\\'hour\\')'],"
"['alter add col','ALTER TABLE sensor ADD COLUMN temp FLOAT64'],"
"['truncate','TRUNCATE TABLE sensor'],"
"['delete range','DELETE FROM sensor WHERE ts < \\'2026-01-01\\''],"
"['drop table','DROP TABLE sensor'],"
"]},{c:'STables & devices',i:["
"['create stable','CREATE STABLE meters (ts TIMESTAMP, volt FLOAT64) TAGS (region SYMBOL, unit INT64)'],"
"['create child','CREATE TABLE d1 USING meters TAGS (\\'east\\', 42)'],"
"['drop stable','DROP STABLE meters'],"
"]},{c:'Cluster catalog',i:["
"['create group','CREATE GROUP factory_a (region=\\'us-east-1\\', retention=\\'30d\\')'],"
"['create device','CREATE DEVICE sensor_001 IN GROUP factory_a (type=\\'temperature\\')'],"
"['list groups','LIST GROUPS'],"
"['list devices','LIST DEVICES IN GROUP factory_a'],"
"['drop device','DROP DEVICE sensor_001 IN GROUP factory_a'],"
"['drop group','DROP GROUP factory_a'],"
"]},{c:'Users & RBAC',i:["
"['create user','CREATE USER alice IDENTIFIED BY \\'secret\\' ROLE normal'],"
"['create admin','CREATE USER root IDENTIFIED BY \\'pw\\' ROLE admin'],"
"['grant select','GRANT SELECT ON sensor TO alice'],"
"['grant ddl','GRANT DDL ON * TO alice'],"
"['grant all','GRANT ALL ON * TO alice'],"
"['revoke','REVOKE INSERT ON sensor FROM alice'],"
"['change pw','ALTER USER alice SET PASSWORD \\'newsecret\\''],"
"['drop user','DROP USER alice'],"
"['list users','LIST USERS'],"
"]},{c:'TMQ (pub/sub)',i:["
"['create cg','CREATE CONSUMER GROUP cg ON sensor'],"
"['join','JOIN GROUP cg AS consumer_1'],"
"['commit offset','COMMIT OFFSET cg AT 1000 AS consumer_1'],"
"['leave','LEAVE GROUP cg AS consumer_1'],"
"]},{c:'UDF',i:["
"['create udf','CREATE FUNCTION my_double(FLOAT64) RETURNS FLOAT64 AS \\'/path/to/udf.so\\' SYMBOL \\'double_fn\\''],"
"['use udf','SELECT ts, my_double(price) FROM trades LIMIT 10'],"
"['list udf','LIST FUNCTIONS'],"
"['drop udf','DROP FUNCTION my_double'],"
"]},{c:'Export',i:["
"['parquet','EXPORT TABLE trades TO PARQUET \\'/tmp/trades.parquet\\''],"
"]}];"
"function renderQex(){"
"const g=document.getElementById('qexgrid');"
"let h='';QEX.forEach(cat=>{"
"h+='<div class=qex-cat><h4>'+esc(cat.c)+'</h4>';"
"cat.i.forEach(it=>{"
"h+=`<div class=qex-item data-sql='${esc(it[1]).replace(/'/g,'&#39;')}' title='click to load'>`"
"+`<div class=t>${esc(it[0])}</div>`"
"+`<pre>${esc(it[1])}</pre>`"
"+'</div>';});"
"h+='</div>';});"
"g.innerHTML=h;"
