...
"en:{"
 ...
 "'common.click_load':'click to load','ident.host':'host',"
...
"zh:{"
 ...
 "'common.click_load':'点击加载','ident.host':'主机',"
...
/* Render tree from cached state.lastTree — called both from loadTree()
...
"let h='';"
"h+='<div class=grp>'+esc(t('common.tables'))+' ('+((tr.tables||[]).length)+')</div>';"
"if(!tr.tables||!tr.tables.length)"
"h+='<div class=empty>'+esc(t('common.no_tables'))+'</div>';"
"else (tr.tables||[]).forEach(x=>{"
"h+=`<div class=item data-tbl='${esc(x.name)}' title='${esc(t('common.click_load'))}'>`"
"+esc(x.name)+'</div>';});"
"if(tr.groups&&tr.groups.length){"
"h+='<div class=grp>'+esc(t('common.groups'))+' ('+tr.groups.length+')</div>';"
"tr.groups.forEach(g=>{"
"h+=`<div class=item data-grp='${esc(g.name)}'>${esc(g.name)}</div>`;});}"
"el.innerHTML=h;"
