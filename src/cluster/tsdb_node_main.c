...
int main(int argc, char **argv) {
    ...
    /* Idle-flush: drain the unflushed memtable tail of quiesced tables that hold
     * locally-ingested rows, so the last <block_points rows reach the shard
     * owners (replicate on flush) and become cluster-visible + durable.  Only
     * local (non-replicated) tails are flushed-with-replicate — received data is
     * never re-replicated. */
    {
        pthread_t idle_thr;
        pthread_attr_t attr;
